...
%         end
        ...
        for testSampleIMU=tsoIMU:tsfIMU
            ...
            while kAux<numExecutionsTimerIMU
                ...
                tClassification=tic;
                [imux,soIMU, sfIMU] = ventanaIMUgforce(idxUser, testSampleIMU, samplesWindowIMU, timeShiftIMU, soIMU, sfIMU);
                %% ############# Recibiendo la señal IMU a la variable ventana #############################################################
                
                
                unknownGestureIMU=imux;
                
                %% ##############           DTW-EMG         ######################################################################################
                
                for kDatasetIMU=1:sizeDatasetIMUs
                    for kChannel=1:4
                ...
                suma=sum(DTWUnknownGestureIMU,1);
                suma=sum(suma,2);
                suma=suma/4;
